...
**Correctif bloquant, création d'un lien privé (0.28.0), premier test réel.** Cliquer sur "Créer un
lien de partage privé" redirigeait vers la liste "Actualités" sans revenir sur la fiche. Cause
racine : les actions Créer/Régénérer/Révoquer étaient rendues comme des `<form>` DANS la boîte
latérale "Partage", elle-même déjà à l'intérieur du grand formulaire d'édition WordPress qui
enveloppe tout l'écran (`<form id="post">`) — un `<form>` imbriqué dans un autre est invalide en
HTML, le clic soumettait donc en réalité le formulaire extérieur (vers `post.php`), jamais notre
gestionnaire `admin-post.php`. Corrigé en remplaçant les formulaires par de simples liens
`<a class="button">` nonce-protégés (`gwseq_horse_private_share_action_url()`, point unique de
construction), exactement le schéma des actions de ligne natives de WordPress. Redirection de
...
**Piloter la diffusion avec le vocabulaire GWS (0.31.0).** Recette du 0.30.0 concluante, mais
l'écran d'édition affichait encore, en parallèle, la boîte native "Publier" (`Brouillon`/`Publier`/
`État`/`Visibilité` Publique-Protégée par mot de passe-Privée) — deux modèles contradictoires. Cette
boîte est désormais remplacée, UNIQUEMENT pour le CPT Cheval (`remove_meta_box('submitdiv',
GWSEQ_CPT_CHEVAL, 'side')`, aucun impact sur Pages/Actualités/Prestations/Membres), par une boîte
"État de diffusion" au vocabulaire métier, avec uniquement les actions pertinentes selon l'état :
*En préparation* → Enregistrer / Activer la diffusion privée / Rendre visible sur le site (si
capacité `publish_post`) ; *Diffusion privée* → Enregistrer les modifications / Rendre visible sur
le site / Repasser en préparation ; *Visible sur le site* → Enregistrer les modifications / retrait
explicite à deux choix (Repasser en préparation OU Activer la diffusion privée — jamais un
"Dépublier" ambigu). Chaque bouton de transition est un vrai `<button type="submit">` du formulaire
d'édition existant : la fiche est réellement enregistrée AVANT la transition, en un seul geste,
via le hook natif `save_post_{cpt}`. Trois nouvelles fonctions métier centralisées et
INDÉPENDANTES de wp-admin (`gwseq_horse_diffusion_set_en_preparation()`/`_diffusion_privee()`/
`_visible_site()`, includes/cheval-share.php) encapsulent chacune la règle complète (statut cible +
gestion du token) — un futur écran mobile appellera exactement ces mêmes fonctions. Sécurité :
capacité `publish_post` vérifiée avant toute mise en visibilité publique (bouton absent ET refus
côté hook de sauvegarde) ; règles du token conservées à l'identique. Jamais le statut natif
`private` de WordPress ni la protection par mot de passe comme implémentation de "Diffusion
privée" (qui reste `draft` + token GWS) : une fiche déjà dans cet état natif reste classée sans
risque selon son état métier réel, et un nouvel audit non destructif (notice sur la liste Chevaux,
`gwseq_cheval_native_visibility_mismatches()`, includes/cheval-fields.php) signale ces cas sans
jamais les migrer automatiquement. Boîte "Partage" : le bouton "Créer" est retiré (centralisé dans
...
(`public => false`, aucune archive, aucun rewrite natif, absente de la recherche/REST) — jamais une
page publique du site. Aucune nouvelle table : la liste ORDONNÉE de chevaux d'une sélection tient
dans une seule meta (`_gwseq_selection_chevaux`, un tableau PHP d'identifiants — l'ordre du tableau
EST l'ordre de présentation, aucun champ d'ordre séparé à synchroniser) ; le titre facultatif est
...
## Diagnostic instrumenté de performance — fiche Cheval (0.36.0, itérations 2 à 4 en 0.37.0/0.38.0/0.39.0)
...
trois instructions est en cause. Toujours aucun correctif à ce stade. Ce fichier est destiné à être
retiré une fois la cause confirmée et corrigée — voir
`tests/gws-equestrian-cheval-perf-diagnostic-test.php` (vérifie explicitement l'absence de tout
changement de comportement) et le `CHANGELOG.md` de ce dossier (0.39.0, 0.38.0, 0.37.0 et 0.36.0)
pour le détail complet.

## Mises en avant (Pop-in / Sticky bar) — retiré (0.21.0)

Un module « Mises en avant » (deux post types, menu dédié, déclenchement/fréquence/ciblage,
rendu front) a été développé et testé en 0.20.0, puis **retiré en 0.21.0 à la suite d'une décision
produit** après recette UX — pas d'un problème technique : cette fonctionnalité est jugée
périphérique à la valeur métier de GWS Equestrian. Décision conservée pour référence future :
*Pop-in / mises en avant temporaires : fonctionnalité périphérique pouvant être couverte par une
extension spécialisée telle que Hustle.* Aucune intégration Hustle (ni dépendance, ni installation
automatique, ni template) n'a été ajoutée — ce sujet sera retraité séparément si l'usage réel le
justifie. Voir `CHANGELOG.md` de ce dossier (0.21.0) pour le détail complet du retrait.
